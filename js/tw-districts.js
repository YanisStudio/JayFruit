/**
 * 台灣縣市／鄉鎮市區對照資料
 * 資料來源：jQuery-TWzipcode（MIT License）整理而來，純靜態資料，
 * 不需要任何外部 API，選單選項完全在瀏覽器本地產生。
 */
window.TW_DISTRICTS = {
 ...
 * @returns {{setValue: function(string, string): void}|null}
 */
window.initTWDistrictSelect = function (citySelectId, districtSelectId) {
    const citySelect = document.getElementById(citySelectId);
    const districtSelect = document.getElementById(districtSelectId);
    if (!citySelect || !districtSelect) return null;

    citySelect.innerHTML = '<option value="">請選擇縣市</option>' +
        Object.keys(window.TW_DISTRICTS).map(function (city) {
            return '<option value="' + city + '">' + city + '</option>';
        }).join('');

    function updateDistricts(selectedDistrict) {
        const city = citySelect.value;
        const districts = window.TW_DISTRICTS[city] || [];
    ...
    return {
        setValue: function (city, district) {
